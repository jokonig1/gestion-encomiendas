...
const Reclamos = () => {
    ...
    const fetchReclamos = async () => {
        try {
            ...
            const url = filtro === 'todos' 
                ? `${API_URL}/api/reclamos`
                : `${API_URL}/api/reclamos?estado=${filtro}`;
    ...
    const handleCambiarEstado = async (id, nuevoEstado) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API_URL}/api/reclamos/${id}/estado`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
        ...
        <DashboardLayout userRole="conserje">
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-gray-800">Reclamos</h2>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                {reclamo.estado !== 'resuelto' && (
                                                    <select
                                                        ...
                                                        onChange={(e) => handleCambiarEstado(reclamo._id, e.target.value)}
                                                        className="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                                    >
